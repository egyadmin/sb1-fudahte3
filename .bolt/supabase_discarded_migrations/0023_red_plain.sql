/*
  ...
  Notes:
    ...
    - Uses variables to store department IDs
*/
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM innovation_opportunities WHERE title_en = 'AI Customer Service') THEN
    ...
    (
      ...
      'تنفيذ خدمة عملاء مدعومة بالذكاء الاصطناعي',
      'high',
      'in_progress'
    ),
    (
  ...
  IF NOT EXISTS (SELECT 1 FROM innovation_suggestions WHERE title_en = 'Mobile App Development') THEN
    INSERT INTO innovation_suggestions (
      title_en,
      title_ar,
      description_en,
      description_ar,
    ...
    (
      ...
      'تطوير تطبيق جوال للعمليات الميدانية',
      'technology',
      'high'
    ),
    (
